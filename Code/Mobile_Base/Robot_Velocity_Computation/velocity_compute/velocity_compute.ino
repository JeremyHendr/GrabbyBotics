 ...
 * Description: Displays the calculated values through a serial connection over bluetooth (COM8) in PuTTy
 * Chose COM8 at 9600 baudrate
 * Allows to compute the velocity of the robot
 * Julius Ortstadt
 * 03.03.2025
*/
...
// Target RPM for both motors (Max gearbox RPM is 100)
int target_RPM = 80;
...
void loop() {
  ...
  if (test){
    ...
    float RPM2_M1 = (M1_tick_count * 60/(interval*pow(10,-3)))/g_shaft_ticks; // RPM of gearbox shaft
    float RPM2_M2 = (M2_tick_count * 60/(interval*pow(10,-3)))/g_shaft_ticks; // RPM of gearbox shaft

    // Compute error (difference in speed)
    float error = RPM2_M1 - RPM2_M2;

    // Compute correction using P controller
    int correction = Kp * error;

    // Adjust motor speeds
    int speed_M1 = constrain(speed - correction, 0, 255);
    int speed_M2 = constrain(speed + correction, 0, 255);

    // Drive the motors forward
    forward(speed_M1, speed_M2);

    // Calculate and send the results via Bluetooth
    if (currentMillis - previousMillis > interval){
      previousMillis = currentMillis;
      stop();
      // Compute and print the measurements
      print_BT();
...
void forward(int speed_M1, int speed_M2){ // Moves forward
  // Motor 1 forward 
  digitalWrite(IN1, LOW);
  digitalWrite(IN2, HIGH);
  analogWrite(ENA, speed_M1);

  // Motor 2 forward
  digitalWrite(IN3, LOW);
  digitalWrite(IN4, HIGH);
  analogWrite(ENB, speed_M2);
}
...
void print_BT(){
  ...
  float RPM1_M1 = (M1_tick_count * 60/(interval*pow(10,-3)))/m_shaft_ticks; // RPM of motor shaft
  float RPM2_M1 = (M1_tick_count * 60/(interval*pow(10,-3)))/g_shaft_ticks; // RPM of gearbox shaft
  ...
  float M1_velocity = (RPM2_M1 * wheel_circumference) / 60.0;
  BlueT.println("Velocity in m/s: " + String(M1_velocity));
  ...
  float RPM1_M2 = (M2_tick_count * 60/(interval*pow(10,-3)))/m_shaft_ticks; // RPM of motor shaft
  float RPM2_M2 = (M2_tick_count * 60/(interval*pow(10,-3)))/g_shaft_ticks; // RPM of gearbox shaft
  ...
  float M2_velocity = (RPM2_M2 * wheel_circumference) / 60.0;
  BlueT.println("Velocity in m/s: " + String(M2_velocity));
