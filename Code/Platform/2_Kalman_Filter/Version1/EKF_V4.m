...
sensor_estimation = zeros(2,length(t));
psi = state_truth(5,1);
a = 1/(pi*D*cos(psi));
b = L/(2*pi*D);
H_temp = [0 a 0 0 0 b;
...
for k = 2:length(t) 
    ...
    state_truth(2,k) = (state_truth(1,k)-state_truth(1,k-1))/Ts;
    state_truth(4,k) = (state_truth(3,k)-state_truth(3,k-1))/Ts;
    state_truth(6,k) = (state_truth(5,k)-state_truth(5,k-1))/Ts;
    
    sensor_estimation(:,k) = H_temp*state_truth(:,k-1);
...
% --- Plot robot trajectory ---
figure; hold on; grid on; legend;
plot(state_truth(1,:), state_truth(3,:), 'b', 'LineWidth', 2, 'DisplayName', 'Estimation EKF');
xlabel('X [m]'); ylabel('Y [m]');
title('Differential Drive Robot Trajectory');
axis equal; grid on;
...
plot(t, state_truth(1,:), 'g', 'DisplayName', 'true x');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('x pos');
xlabel('Time [s]');
ylabel('x [m/s]');
...
plot(t, state_truth(3,:), 'g', 'DisplayName', 'true x');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('x pos');
xlabel('Time [s]');
ylabel('x [m/s]');
...
plot(t, state_truth(2,:), 'g', 'DisplayName', 'true x speed');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('x speed');
xlabel('Time [s]');
ylabel('x [m/s]');
...
plot(t, state_truth(4,:), 'g', 'DisplayName', 'true y speed');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('y speed');
ylim([-2 2]); 
xlabel('Time [s]');
...
plot(t, state_truth(5,:), 'g', 'DisplayName', 'true x speed');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('psi pos');
ylim([-3 1]); 
xlabel('Time [s]');
...
plot(t, state_truth(6,:), 'g', 'DisplayName', 'true x speed');
% plot(t, encoder_right, 'b', 'DisplayName', 'encoder data');
title('psi speed');
xlabel('Time [s]');
ylabel('x [m/s]');
...
subplot(2,1,1); hold on; grid on;
plot(t, wR, 'g', 'DisplayName', 'true wheel speed');
plot(t, sensor_estimation(1,:), 'b', 'DisplayName', 'encoder data');
ylim([3 7]);
title('Right wheel speed');
xlabel('Time [s]');
ylabel('Speed [m/s]');
...
subplot(2,1,2); hold on; grid on;
plot(t, wL, 'g', 'DisplayName', 'true wheel speed');
plot(t, sensor_estimation(2,:), 'b', 'DisplayName', 'encoder data');
ylim([3 7]);
title('Right wheel speed');
xlabel('Time [s]');
ylabel('Speed [m/s]');
