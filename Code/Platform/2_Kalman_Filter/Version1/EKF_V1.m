...
X = [0;
     ...
     0;
     0;
     0]; % State vector x, xdot, y, ydot, yaw, yawdot
...
H = [0 0;
    wheel_diameter*cos()]; % sensor_vec to state_vec relation matrix

P = diag(ones(size(X))).*(10^9); % Initial estimate covariance matrix

R = measurement_noise_deviation^2; % measurement noise cov matrix

phi = [ 1 Ts (Ts^2)/2;
        0 1   Ts;
        0 0   1];     %Discrete time transition matrix
