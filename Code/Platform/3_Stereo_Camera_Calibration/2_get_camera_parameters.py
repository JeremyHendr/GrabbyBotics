...
def __main__():
    ...
    matrix_right, distortion_coef_right, valid_indices_right = calibrate_camera(images_names_right, rows, columns, 1)
    print("right camera calibrated")
    matrix_left, distortion_coef_left, valid_indices_left = calibrate_camera(images_names_left, rows, columns, 1)
    print("left camera calibrated")

    #Find common valid indices (images where corners were found in both cameras)
    valid_indices = list(set(valid_indices_right) & set(valid_indices_left))
    valid_indices.sort()
    print(f"\nUsing {len(valid_indices)} image pairs for stereo calibration (out of {len(images_names_right)} total)")
    
    #Filter image lists to only include valid pairs
    filtered_images_right = [images_names_right[i] for i in valid_indices]
    filtered_images_left = [images_names_left[i] for i in valid_indices]
    ...
    rotation_matrix, translation_vector = stereo_calibrate(matrix_right, distortion_coef_right, matrix_left, distortion_coef_left, filtered_images_right, filtered_images_left, rows, columns,1)
...
def calibrate_camera(images_names, rows, columns, display_time=10):
    ...
    Returns:
        ...
        valid_indices (list): list of indices where corners were found
    """
    ...
    #Track which images had corners found
    valid_indices = []
    ...
    return camera_matrix, distortion_coefficients, valid_indices
...
def stereo_calibrate(mtx1, dist1, mtx2, dist2, frames_names_c1, frames_names_c2, rows, columns, display_time=10):
    ...
    for frame_c1, frame_c2 in zip(c1_gray_frames, c2_gray_frames):
        ...
        if ret_c1 != ret_c2:
            print( "corners not found in a files check if there is a picture where no corners have been found")
