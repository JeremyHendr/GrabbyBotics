...
ros::Publisher rightPub("m1_ticks", &right_wheel_tick_count);

std_msgs::Int16 left_wheel_tick_count;
ros::Publisher leftPub("m2_ticks", &left_wheel_tick_count);
...
const int K_P = 814.58;

// Y-intercept for the PWM-Linear Velocity relationship for the robot
const int b = -94.74;

// Correction multiplier for drift. Chosen through experimentation.
const int DRIFT_MULTIPLIER = 0;

// Turning PWM output (0 = min, 255 = max for PWM values)
const int PWM_TURN = 80;

// Set maximum and minimum limits for the PWM values
const int PWM_MIN = 80; // about 0.2123 m/s
const int PWM_MAX = 100; // about 0.2363 m/s
...
void calc_vel_left_wheel(){
  ...
  float RPM2_M1 = (numOfTicks * 60/((millis()-prevTime)*pow(10,-3)))/GEAR_BOX_SHAFT_TICKS; // RPM of gearbox shaft
  velLeftWheel = (RPM2_M1 * WHEEL_CIRCUMFERENCE) / 60.0;
...
void calc_vel_right_wheel(){
  ...
  // Calculate wheel velocity in meters per second
  float RPM2_M2 = (numOfTicks * 60/((millis()-prevTime)*pow(10,-3)))/GEAR_BOX_SHAFT_TICKS; // RPM of gearbox shaft
  velRightWheel = (RPM2_M2 * WHEEL_CIRCUMFERENCE) / 60.0;
...
void calc_pwm_values(const geometry_msgs::Twist& cmdVel) {
  ...
  // Go straight
    ...
    // Correction factor for motor imbalance
    const double CORRECTION_FACTOR = 5; // Tune this experimentally

    // Apply correction to only one motor (e.g., the right one)
    pwmLeftReq +=  CORRECTION_FACTOR;
  }
